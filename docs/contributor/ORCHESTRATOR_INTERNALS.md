...
> This document covers the internal architecture of the xchecker engine.
> For a high-level overview of how xchecker works, see [Architecture](../explanation/ARCHITECTURE.md).
...
**Code Location:** `crates/xchecker-engine/src/orchestrator/llm.rs`
...
**Code Location:** `crates/xchecker-engine/src/orchestrator/phase_exec.rs`
...
**Code Location:** `crates/xchecker-engine/src/orchestrator/workflow.rs`

### `handle.rs` -- Safe Facade for External Callers

Provides a clean, stable API for agents and external tools to interact with the orchestrator.
...
**Code Location:** `crates/xchecker-engine/src/orchestrator/handle.rs`
...
|  (Public API - CLI, agents, external tools)                       |
+-----------------------------+------------------------------------+
                              |
                              v
+------------------------------------------------------------------+
...
- `Config::strict_validation()` in `crates/xchecker-config/src/config.rs` -- reads from config file
...
2. `PhaseContext.strict_validation` is populated in `create_phase_context()` (`crates/xchecker-engine/src/orchestrator/phase_exec.rs`)
...
See `crates/xchecker-utils/src/exit_codes/codes.rs` for complete list.
...
- **`crates/xchecker-engine/src/orchestrator/mod.rs`**: Unit tests within orchestrator module itself
...
**Context**: These were exposed for interop during migration. With the facade pattern established, they may be narrower than necessary.
