...
You have a feature idea and want a structured path from requirements through
implementation to a merged pull request. This tutorial walks through the
complete xchecker workflow: create a spec, generate requirements/design/tasks
with an LLM, implement the feature, validate with gates, and submit a PR with
CI checks enforcing spec quality.

**Time**: 30--45 minutes (depending on LLM response times)

**Prerequisites**:
- xchecker installed and configured (see [Quickstart](QUICKSTART.md))
- Claude Code environment set up (optional, for IDE integration)
...
```bash
cat .xchecker/specs/user-auth/artifacts/20-tasks.md
```
...
const design = await readFile(".xchecker/specs/user-auth/artifacts/10-design.md");
```
...
Add xchecker gate checks to your CI pipeline. See [CI Setup](../guides/CI_SETUP.md) for complete configuration.
...
Here is a script that demonstrates the full flow:
...
echo "View tasks: cat .xchecker/specs/$SPEC_ID/artifacts/20-tasks.md"
```
...
cat .xchecker/specs/user-auth/artifacts/10-design.md
...
### Claude Code Cannot Parse JSON

Ensure you are using the `--json` flag:
...
- [Quickstart](QUICKSTART.md) -- install and first spec in 20 minutes
- [Claude Code Integration](../guides/CLAUDE_CODE.md) -- detailed Claude Code setup
- [Configuration Guide](../guides/CONFIGURATION.md) -- full configuration how-to
- [CI Setup](../guides/CI_SETUP.md) -- CI/CD pipeline integration
