...
use presentation::{styled_check, styled_info, styled_success, styled_warning};
...
pub fn run() -> Result<(), ExitCode> {
    ...
    let result = rt.block_on(async {
        match cli.command {
            ...
            Commands::Project(project_cmd) => project::execute_project_command(project_cmd),
            Commands::Gate {
                id,
                policy,
                min_phase,
            ...
            Commands::Template(template_cmd) => template::execute_template_command(template_cmd),
        }
    });
